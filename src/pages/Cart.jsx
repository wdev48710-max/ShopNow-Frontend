...
const Cart = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-darkColor mb-6">Shopping Cart</h1>
      {cart.length === 0 ? (
        ...
        <>
            ...
            <div key={item._id} className="bg-white rounded-lg shadow p-4 mb-4 flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-darkColor">{item.name}</h3>
                  <p className="text-darkBlue font-bold">${item.price}</p>
                </div>
              </div>
              <div className="flex items-center justify-between sm:justify-end gap-4 flex-1">
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => item.quantity > 0 && updateQuantity(item._id, item.quantity - 1)}
                ...
                <p className="font-bold text-darkColor">${(item.price * item.quantity).toFixed(2)}</p>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3 mt-6">
            <p className="text-2xl font-bold text-darkColor text-right">Total: ${totalPrice.toFixed(2)}</p>
            <div className="flex gap-3 sm:ml-4">
              ...
              <button onClick={() => navigate("/checkout")} className="px-6 py-2 bg-darkBlue text-white rounded hover:bg-lightBlue">Proceed to Checkout</button>
            </div>
          </div>
        </>
      )}
