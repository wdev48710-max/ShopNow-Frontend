    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="bg-white rounded-xl shadow overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="p-6 flex-1">
          <h1 className="text-3xl font-bold text-darkColor mb-2">{product.name}</h1>
          <p className="text-2xl font-bold text-darkBlue mb-4">${product.price}</p>
          <p className="text-gray-500 mb-2">{product.category}</p>
          <p className="text-gray-700 mb-2">{product.description}</p>
          <button
            onClick={() => addToCart(product)}
            className="px-6 py-3 bg-darkColor text-white rounded-lg hover:bg-darkBlue transition cursor-pointer"
          >
            Add to cart
          </button>
        </div>
